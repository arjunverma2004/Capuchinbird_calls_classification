...
def load_model():
    ...
    try:
        # Ensure your TRAINED model is named this and is in the root directory
        model_path = 'models/calls_classifier.keras'
        if not os.path.exists(model_path):
            st.error(f"Model file not found at '{model_path}'. Please ensure the correct model is uploaded.")
            st.stop()
...
def create_spectrogram(wav_slice):
    ...
    # Normalize the spectrogram to the [0, 1] range
...
if uploaded_file is not None:
    ...
    if duration is None:
        ...
        st.write(f"(Using a fixed prediction threshold of {threshold})")

        with st.expander("Show Detailed Prediction Scores"):
            st.write("These are the model's confidence scores for each 3-second segment. Scores above the threshold are counted as a call.")
